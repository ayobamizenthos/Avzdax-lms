import type { Metadata } from "next";
import { Sparkles } from "lucide-react";

import { Wordmark } from "@/components/brand/wordmark";
import { FoundationsForm } from "./foundations-form";

export const metadata: Metadata = {
  title: "Foundations Programme",
};

export default function FoundationsPage() {
  return (
    <div className="min-h-dvh bg-paper px-5 py-12 sm:py-16">
      <div className="mx-auto w-full max-w-lg animate-rise">
        <div className="mb-8 flex justify-start sm:justify-center">
          <div className="sm:hidden">
            <Wordmark size="sm" />
          </div>
          <div className="hidden sm:block">
            <Wordmark />
          </div>
        </div>

        <div className="rounded-xl bg-surface p-6 shadow-card sm:p-8">
          <div className="text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand">
              AVZDAX Foundations Programme
            </p>
            <h1 className="mt-2 font-display text-2xl text-ink">
              Continue your journey with AVZDAX
            </h1>
            <p className="mt-2 text-sm text-ink-soft">
              A practical, instructor-led programme that builds the technical and
              problem-solving foundations you need to thrive in technology.
            </p>
          </div>

          <div className="my-6 flex items-center justify-center gap-2 rounded-sm bg-gold-tint px-4 py-2.5 text-sm text-gold">
            <Sparkles className="size-4" />
            Programme fee ₦35,000, exclusive to applicants from this cohort.
          </div>

          <FoundationsForm />
        </div>
      </div>
    </div>
  );
}
